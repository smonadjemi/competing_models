...
some constants
"""
DATA_DIRECTORY = 'outputs/model_posteriors_35'
...
for task in TASK_TO_QUESTION_TO_PARTICIPANTS.keys():
    ...
    for question in TASK_TO_QUESTION_TO_PARTICIPANTS[task].keys():
        ...
        for participant in TASK_TO_QUESTION_TO_PARTICIPANTS[task][question]:
            ...
            with open('%s/target_analysis_%s_%s_%d.json' % (DATA_DIRECTORY, task, question, participant), 'r') as file:
                target_data = json.load(file)
                number_of_clicks[question].append(len(target_data))

                for time in target_data.keys():
                    if time not in metrics[question].keys():
                        metrics[question][time] = {m: [] for m in target_data[time].keys()}

                    if time not in aggregate_posterior.keys():
                        aggregate_posterior[time] = []
                    for m in target_data[time].keys():
                        metrics[question][time][m].append(target_data[time][m])

                    aggregate_posterior[time].append(target_data[time]['recall'])
...
aggregate_posterior_means = np.array([np.mean(aggregate_posterior[str(t)]) for t in range(2, len(aggregate_posterior.keys()))])
aggregate_posterior_sd = np.array([np.std(aggregate_posterior[str(t)]) for t in range(2, len(aggregate_posterior.keys()))])
...
plt.ylabel("Recall")
plt.title("Aggregate Recall across all Tasks")
...
posterior_means = {question: {model: [np.mean(metrics[question][str(time)][model]) for time in range(2, len(metrics[question]))] for model in metrics[question]["2"]} for question in metrics.keys()}
posterior_sd = {question: {model: [np.std(metrics[question][str(time)][model]) for time in range(2, len(metrics[question]))] for model in metrics[question]["2"]} for question in metrics.keys()}
...
for question in QUESTION_TO_TASK.keys():
    ...
    for m in ['recall', 'precision']:
        mean = np.array(posterior_means[question][m])
        sd = np.array(posterior_sd[question][m])
        plt.plot(mean, label=m)
        plt.fill_between(list(range(len(mean))), mean-sd, mean+sd, alpha=0.2)
    plt.ylim([-0.1, 1.1])
    plt.xlabel("# of clicks")
    plt.title("Question %s - %s" % (question, task_type))
    plt.legend()
