...
def plot_data():
    ...
    xs = np.linspace(min(data['lat']) - eps_lat, max(data['lat']) + eps_lat, 100)
    ys = np.linspace(min(data['lng']) - eps_lng, max(data['lng']) + eps_lng, 100)
    p = [[0 for x in xs] for y in
         ys]

    ax_loc = fig.add_subplot(gs[0:2, 0:2])
    ax_loc.scatter(data['lat'], data['lng'], s=3, c=colors, marker='o')
    ax_loc.imshow(p, extent=(min(xs), max(xs), min(ys), max(ys)), aspect='equal', alpha=0.8, origin='lower',
                  cmap=get_my_cmap())

    ax_loc.set_title(f'location - full data', fontsize=18)
    ax_loc.set_xlabel('latitude')
    ax_loc.set_ylabel('longitude')
    ax_loc.set_xticks([])
    ax_loc.set_yticks([])
    ...
    ax_type.set_title(f'type - full data', fontsize=18)
    ...
    ax_type.xaxis.set_tick_params(rotation=90)
...
def plot(t):
    ...
    xs = np.linspace(min(data['lat']) - eps_lat, max(data['lat']) + eps_lat, 100)
    ys = np.linspace(min(data['lng']) - eps_lng, max(data['lng']) + eps_lng, 100)
    p = [[cmodel.get_probability(np.array([(x, y)], dtype=[('lat', 'float'), ('lng', 'float')])) for x in xs] for y in ys]

    ax_loc = fig.add_subplot(gs[0:2, 0:2])
    ax_loc.scatter(data['lat'], data['lng'], s=3, c=colors)

    mouse = parse_path(MOUSE_ICON)
    mouse.vertices[:, 1] *= -1
    # mouse.vertices -= mouse.vertices.mean(axis=0)

    ax_loc.scatter(this_ui_data['lat'][:t], this_ui_data['lng'][:t], marker=mouse, c='black', s=500, alpha=0.5)
    ax_loc.set_title(f'location model at t={t}', fontsize=18)
    ax_loc.set_xlabel('latitude')
    ax_loc.set_ylabel('longitude')
    ax_loc.set_xticks([])
    ax_loc.set_yticks([])
    ...
    ax_type = fig.add_subplot(gs[2, 0:2])
    ax_type.set_title(f'type model at t={t}', fontsize=18)
    ax_type.spines['right'].set_visible(False)
    ax_type.spines['top'].set_visible(False)
    ax_type.set_yticks([0, 0.5, 1])
    ...
    plt.savefig('figures/%s_%d.png' % (t, int(random.random()*1000)), bbox_inches = 'tight',
    pad_inches = 0.1)
    plt.show()
...
task_type = 'mixed'
ui_data_full = Data.load_ui_data('crime', task_type='mixed')
participant = 19
...
for interaction_index in range(len(this_ui_data)):
    ...
    if interaction_index in [4, 9, 19, 29]:
